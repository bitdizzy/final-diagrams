...
quadForm a b c = if' (a %== 0)
  (if' (b %== 0)
    (if' (c %== 0) nothing' (just' nil'))
    (just' (pure' (negate' c %/ b)))
  )
  ( let d = b %^ (2 :: repr Int) - 4 %* a %* c
        q = (-1) %/ 2 %* (b %+ signum' b %* sqrt d)
     in ordering' (compare' d 0)
          (just' nil')
          (if' (b %== 0)
             (just' $ val1 [sqrt' (negate c %/ a), negate (sqrt' (negate c %/ a))])
             (just' $ pure' (negate b %/ 2%* a))
          )
          (just' $ val1 $ [q %/ a, c %/ q])
  )

aboutZero' :: (Ord' repr a, Num' repr a) => repr a -> repr a -> repr Bool
aboutZero' tolerance x = abs' x %<= tolerance

belowZero' :: (Ord' repr a, Num' repr a) => repr a -> repr a -> repr Bool
...
cubForm a btimes3 ctimes3 d =
  let b = btimes3 / 3
      ...
      branch1 =
        ...
        let depressionTest = b %^ 3 %* d %>= a %* (c %^ 3)
            ...
            x1 = if' (c' %<= 0) (p %+ q) (negate d' %/ (p %^ 2 + q %^ 2 + c'))
      ...
      branch2 =
        let theta a' d' = (1/3) %* abs' (atan2' (a' %* sqrt' disc) (negate' d'))
            x1 sqrtc' costheta' = 2 %* sqrtc' %* costheta'
            x3 sqrtc' costheta' sintheta' =
              2 %* sqrtc' %* ((-1/2) %* costheta' %- (sqrt 3 / 2) %* sintheta')
                 ...
                 let_ (x3 sqrtc_max costheta_max sintheta_max) \x3_max ->
                  let_ (x1 sqrtc_min costheta_min) \x1_min ->
                   let_ (x3 sqrtc_min costheta_min sintheta_min) \x3_min ->
                    ...
                    let rootMax_x = if' (x1_max %+ x3_max %> 2 %* b) x1_max x3_max
                        rootMax_w = a
                        rootMin_x = negate' d
                        rootMin_w = if' (x1_min %+ x3_min %< 2 %* c) x1_min x3_min
   ...
   -- even tiny a's.
   in if' (a %== 0) (quadForm b c d) (if' (disc %< 0) branch1 branch2)
